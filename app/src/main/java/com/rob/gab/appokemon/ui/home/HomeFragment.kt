...
class HomeFragment : Fragment() {
    ...
    private fun observeState() {
        lifecycleScope.launch {
            viewModel.state.collectLatest {
                when (it) {
                    //Actually we just have a single Intent
    ...
    private fun initComponents() {
        ...
        mAdapter.addLoadStateListener { loadState ->
            ...
            errorState?.let {
                FloatingToastDialog(requireContext(),"An error occurred. Please retry again.", FloatingToastDialog.FloatingToastType.Error).show()
            }
        }
